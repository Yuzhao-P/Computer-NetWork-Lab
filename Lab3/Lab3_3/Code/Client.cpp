...
int sended_notacked(int base, int nextseqnum1) {
	int i = 0;
	for (int j = base; j < nextseqnum1; j++) {
		if (!acked[j])
			i++;
	}
	return i;
	...
	while (1) {
		...
		if (recvByte > 0) {  // 成功收到消息【不管是啥，收到东西了】
			if ((recvMsg.Flag && ACK) && recvMsg.check()) {  // 检查校验和
				...
				// outputMutex.lock();  // 手动加锁
				if (recvMsg.ack >= base && recvMsg.ack <= base + N - 1) {  // 在窗口内
					// 更新窗口状态
					if (recvMsg.ack == base) {
						acked[base] = true;
						while (acked[base]) {
						...
						if (base + N < pkt_amount) {  // 展示窗口情况
							cout << "[传输日志] [接收ACK]当前窗口情况——窗口总大小：" << N << "，已发送但未收到ACK的数据报数目：" << sended_notacked(base, nextseqnum) << "，窗口内尚未发送数据报数目：" << N - (nextseqnum - base) << endl;
						}
						else {  // 展示窗口情况
							cout << "[传输日志] [接收ACK]当前窗口情况——窗口总大小：" << N << "，已发送但未收到ACK的数据报数目：" << sended_notacked(base, nextseqnum) << "，窗口内尚未发送数据报数目：" << pkt_amount - base - (nextseqnum - base) << endl;
						}
						outputMutex.unlock();  // 手动解锁
					}
					else {
						acked[recvMsg.ack] = true;
						outputMutex.lock();  // 手动加锁
						cout << "[传输日志] " << recvMsg.SrcPort << " -> " << recvMsg.DestPort << " ack = " << recvMsg.ack << " Flag = " << recvMsg.Flag << endl;
						outputMutex.unlock();  // 手动解锁
						//if (base + N < pkt_amount) {  // 展示窗口情况
...
// 报文传输函数
void SR_SendFunc(string path, SOCKET clientSocket, SOCKADDR_IN serverAddr) {
	int addrlen = sizeof(serverAddr);
	...
	while (1) {
		...
		if (nextseqnum < base + N && nextseqnum < pkt_amount) {
			...
			timers[nextseqnum] = clock();  // 给每一个包设置计时器
			nextseqnum++;
			outputMutex.lock();  // 手动加锁
			if (base + N < pkt_amount) {  // 展示窗口情况
				cout << "[传输日志] 当前窗口情况——窗口总大小：" << N << "，已发送但未收到ACK的数据报数目：" << sended_notacked(base, nextseqnum) << "，窗口内尚未发送数据报数目：" << N - (nextseqnum - base) << endl;
			}
			else {  // 展示窗口情况
				cout << "[传输日志] 当前窗口情况——窗口总大小：" << N << "，已发送但未收到ACK的数据报数目：" << sended_notacked(base, nextseqnum) << "，窗口内尚未发送数据报数目：" << pkt_amount - base - (nextseqnum - base) << endl;
			}
			outputMutex.unlock();  // 手动解锁
		}
		//outputMutex.unlock();  // 手动解锁
		...
		for (int i = 0; i < nextseqnum - base; i++) {
			int send_seq = base + i;
			if (!acked[send_seq] && ((clock() - timers[send_seq]) > MAX_WAIT_TIME)) {  // SR——超时包重传
				if (send_seq == 0) {
					sendMsg.SrcPort = ClientPORT;
					sendMsg.DestPort = RouterPORT;
					sendMsg.size = fileSize;
...
int main() {
	...
	while (res) {
		...
		if (c == 'r' || c == 'R')
		{
			...
			SR_SendFunc(filepath, clientSocket, serverAddr);
		}
		else if (c == 'q' || c == 'Q')
		{
			res = false;  // 退出循环
